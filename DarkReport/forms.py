...
class FindForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            'reconnaissance': forms.Textarea(attrs={'rows': 6, 'class': 'form-control', 'placeholder': _('How discovery was done, Nmap, Nuclei...')}),
            'weaponization': forms.Textarea(attrs={'rows': 1, 'class': 'form-control', 'placeholder': _('Which software can be used for attack')}),
            'delivery': forms.Textarea(attrs={'rows': 1, 'class': 'form-control', 'placeholder': _('How the payload is delivered, POST, mail...')}),
            'exploitation': forms.Textarea(attrs={'rows': 1, 'class': 'form-control', 'placeholder': _('A brief explanation of the vulnerability')}),
            ...
            'actions': forms.Textarea(attrs={'rows': 6, 'class': 'form-control', 'placeholder': _('Which actions can be performed after the exploit')}),
    ...
    def __init__(self, *args, **kwargs):
        ...
        value = self.initial.get("actions")
        ...
        for name, field in self.fields.items():
            ...
            if value:
                rows = min(max(len(value.splitlines()) + 2, 4), 20)
            else:
                rows = 3
